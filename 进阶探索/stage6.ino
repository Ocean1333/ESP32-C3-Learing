...
xQueueHandle queueMsg4 = xQueueCreate(1, sizeof(int));
// 信号量
SemaphoreHandle_t mutex = NULL; // 创建信号量
char mqttBuf[256];
// Buzzer
int key[8] = {NOTE_E5, NOTE_D5, NOTE_C5, NOTE_D5, NOTE_E5, NOTE_F5, NOTE_E5, NOTE_D5};
int duration[8] = {620, 310, 620, 310, 465, 155, 310, 620};
...
void handleLED(void* arg){
    ...
    while (1)
    {
        if (xQueueReceive(queueMsg3, buf, 1000) == pdPASS)
        {
            ...
            {
                digitalWrite(ledPin, HIGH); //开灯时通过队列发送消息，控制蜂鸣器播放音乐
                int t = 1;
                xQueueSend(queueMsg4, &t, 100);
            }
...
void handleBuzzer(void* arg){
    int buf;
    pinMode(buzzerPin, OUTPUT);
    while (1)
    {
        if (xQueueReceive(queueMsg4, &buf, 1000) == pdPASS)
        {
            for (unsigned int i = 0; i < sizeof(key)/sizeof(int); i++)
            {
                tone(buzzerPin, key[i], duration[i] * 0.9);
...
void setup(){
    ...
    xTaskCreate(handleBuzzer, "Buzzer", 1024*4, NULL, 1, NULL);
}
